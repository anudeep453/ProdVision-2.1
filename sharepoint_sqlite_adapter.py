...
class SharePointSQLiteAdapter:
    ...
    def migrate_database(self):
        ...
        try:
            ...
            # Add related_issue columns if missing
            cursor.execute("PRAGMA table_info(prbs)")
            prb_cols = [row[1] for row in cursor.fetchall()]
            if 'related_issue' not in prb_cols:
                cursor.execute('ALTER TABLE prbs ADD COLUMN related_issue TEXT')
            cursor.execute("PRAGMA table_info(hiims)")
            hiim_cols = [row[1] for row in cursor.fetchall()]
            if 'related_issue' not in hiim_cols:
                cursor.execute('ALTER TABLE hiims ADD COLUMN related_issue TEXT')
    ...
    def get_entries_by_application(self, application_name: str) -> List[Dict]:
        ...
        for row in cursor.fetchall():
            ...
            for r in cursor.fetchall():
                prbs.append({
                    'id': r[0], 'prb_id_number': r[1], 'prb_id_status': r[2], 'prb_link': r[3], 'related_issue': r[4], 'position': r[5], 'created_at': r[6]
                })
            entry['prbs'] = prbs
            ...
            for r in cursor.fetchall():
                hiims.append({
                    'id': r[0], 'hiim_id_number': r[1], 'hiim_id_status': r[2], 'hiim_link': r[3], 'related_issue': r[4], 'position': r[5], 'created_at': r[6]
                })
            entry['hiims'] = hiims
    ...
    def get_all_entries(self) -> List[Dict]:
        ...
        for row in cursor.fetchall():
            ...
            prbs = []
            for r in cursor.fetchall():
                prbs.append({'id': r[0], 'prb_id_number': r[1], 'prb_id_status': r[2], 'prb_link': r[3], 'related_issue': r[4], 'position': r[5], 'created_at': r[6]})
            entry['prbs'] = prbs

            cursor.execute('SELECT id, hiim_id_number, hiim_id_status, hiim_link, related_issue, position, created_at FROM hiims WHERE entry_id = ? ORDER BY position ASC, id ASC', (entry_id,))
            hiims = []
            for r in cursor.fetchall():
                hiims.append({'id': r[0], 'hiim_id_number': r[1], 'hiim_id_status': r[2], 'hiim_link': r[3], 'related_issue': r[4], 'position': r[5], 'created_at': r[6]})
            entry['hiims'] = hiims
