...
let isSubmitting = false; // Prevent multiple concurrent form submissions

// Register Chart.js plugins
Chart.register(ChartDataLabels);

// Form submission protection functions
function startSubmission(form) {
    if (isSubmitting) {
        return false; // Already submitting, block this attempt
    }
    isSubmitting = true;
    ...
    if (submitBtn) {
        ...
        submitBtn.textContent = submitBtn.textContent.replace('Save', 'Saving...');
    }
    
    return true; // Submission allowed
}

function endSubmission(form, success = true) {
    isSubmitting = false;
    ...
    if (submitBtn) {
        ...
        submitBtn.textContent = submitBtn.textContent.replace('Saving...', 'Save');
    }
    
    // Clear form if successful
    if (success) {
        form.reset();
        // Clear any dynamically added items
        const combinedContainer = form.querySelector('.combined-items-container, #combined-items-container, #xva-combined-items-container');
        if (combinedContainer) {
            combinedContainer.innerHTML = '';
        }
        // Mark form as clean if the function exists
        if (typeof markFormClean === 'function') {
            markFormClean();
        } else {
            // Reset form dirty flag if it exists
...
function getVisibleColumnsForApplication(application) {
    ...
    const othersOnlyColumns = ['business_chain', 'timings', 'puntuality_issue', 'quality', 'quality_issue', 'others_prb', 'others_hiim']; // business_chain should be visible
...
function toggleColumnsForApplication(application) {
    ...
    // Define columns to show only for OTHERS
    const othersOnlyColumns = ['business_chain', 'timings', 'puntuality_issue', 'quality', 'quality_issue', 'others_prb', 'others_hiim'];
    ...
    const othersHiddenColumns = ['prc_mail', 'cp_alerts', 'quality_status', 'remarks', 'issue_description', 'time_loss', 'prb_id', 'hiim_id', ...xvaOnlyColumns, ...regOnlyColumns].filter(col => col !== 'business_chain');
...
async function loadData() {
    ...
    // Load entries only
    const entriesResponse = await fetch('/api/entries?' + buildQueryString() + '&_t=' + Date.now());
    const entries = await entriesResponse.json();        if (entriesResponse.ok) {
...
function displayEntries(entries) {
    ...
    // Detect if we're in row-level filtering mode (exactly one of PRB Only, HIIM Only, or Time Loss Only)
    // Row-level filtering shows individual database rows as separate table rows
...
function createEntryRow(entry, isFirstRow, itemType, entryId, childCount) {
    ...
    // - If entry has no time_loss but is first row and NO entries have time_loss → show 'N/A'
    // - If entry has no time_loss but is NOT first row → show 'N/A' (each itemset should show its own value or N/A)
    // - Otherwise → show 'N/A' (changed from empty to always show N/A for empty fields)
    const timeLossDisplay = entry.time_loss ? escapeHtml(entry.time_loss) : 'N/A';
    ...
    const columnContent = {
        ...
        // OTHERS columns
    business_chain: { html: businessChainDisplay, cls: 'common-field' },
...
async function handleCVAREntrySubmit(event) {
    ...
    if (!startSubmission(cvarEntryForm)) {
        return; // Already submitting, ignore this attempt
    }
    ...
    try {
        ...
        if (validationErrors.length > 0) {
            ...
            endSubmission(cvarEntryForm, false);
            return;
        }
    ...
    // response status debug removed
        ...
        if (response.ok) {
            ...
            endSubmission(cvarEntryForm, true); // Mark as successful
        } else {
            ...
            endSubmission(cvarEntryForm, false); // Mark as failed
        }
    } catch (error) {
...
async function handleXVAEntrySubmit(event) {
    ...
    // Prevent multiple concurrent submissions
    if (!startSubmission(xvaEntryForm)) {
        return; // Already submitting, ignore this attempt
    }
    ...
    try {
        ...
        if (validationErrors.length > 0) {
            alert('Validation Error:\n\n' + validationErrors.join('\n'));
            endSubmission(xvaEntryForm, false);
            return;
        }
    ...
    // response status debug removed
        ...
        if (response.ok) {
            ...
            endSubmission(xvaEntryForm, false); // Mark as failed
        }
    } catch (error) {
...
async function handleREGEntrySubmit(event) {
    ...
    // Prevent multiple concurrent submissions
    if (!startSubmission(regEntryForm)) {
        return; // Already submitting, ignore this attempt
    }
    ...
    const response = await fetch(url, {
        ...
        if (response.ok) {
            hideEntryModal();
            loadData(); // Refresh data
            endSubmission(regEntryForm, true); // Mark as successful
        } else {
            const error = await response.json();
            if (error.error && error.error.includes('already exists')) {
                alert('Error: ' + error.error + '\n\nPlease edit the existing entry or choose a different date.');
            ...
            endSubmission(regEntryForm, false); // Mark as failed
        }
    } catch (error) {
...
async function handleOTHERSEntrySubmit(event) {
    ...
    // Prevent multiple concurrent submissions
    if (!startSubmission(othersEntryForm)) {
        return; // Already submitting, ignore this attempt
    }
    ...
    const response = await fetch(url, {
        ...
        if (response.ok) {
            hideEntryModal();
            loadData(); // Refresh data
            endSubmission(othersEntryForm, true); // Mark as successful
        } else {
            const error = await response.json();
            if (error.error && error.error.includes('already exists')) {
                alert('Error: ' + error.error + '\n\nPlease edit the existing entry or choose a different date.');
            ...
            endSubmission(othersEntryForm, false); // Mark as failed
        }
    } catch (error) {
...
async function deleteEntry(entryId) {
    ...
    try {
        ...
        alert('Failed to delete entry. Please check if the server is running and try again.');
    }
}
...
function initializeColumnReordering() {
    ...
    if (savedOrder) {
        try {
            ...
            const allColumns = ['date', 'day', 'prc_mail', 'cp_alerts', 'quality_status', 'issue_description', 'prb_id', 'hiim_id', 'remarks', 'acq', 'valo', 'sensi', 'cf_ra', 'quality_legacy', 'quality_target', 'root_cause_application', 'root_cause_type', 'xva_remarks', 'business_chain', 'timings', 'puntuality_issue', 'quality', 'quality_issue', 'others_prb', 'others_hiim', 'actions'];
